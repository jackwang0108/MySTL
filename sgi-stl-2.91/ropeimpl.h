...
void __rope_iterator_base<charT, Alloc>::setbuf(__rope_iterator_base<charT, Alloc> &x) {
...
template<class charT, class Alloc>
void __rope_iterator_base<charT, Alloc>::setcache(__rope_iterator_base<charT, Alloc> &x) {
...
template<class charT, class Alloc>
void __rope_iterator_base<charT, Alloc>::setcache_for_incr(__rope_iterator_base<charT, Alloc> &x) {
...
template<class charT, class Alloc>
inline __rope_const_iterator<charT, Alloc>::__rope_const_iterator(const __rope_iterator<charT, Alloc> &x)
    : __rope_iterator_base<charT, Alloc>(x) {}

template<class charT, class Alloc>
inline __rope_iterator<charT, Alloc>::__rope_iterator(rope<charT, Alloc> &r, size_t pos)
...
rope<charT, Alloc>::RopeFunction_from_fn(char_producer<charT> *fn, size_t size, bool delete_fn) {
	if (0 == size) return 0;
...
template<class charT, class Alloc>
rope<charT, Alloc>::RopeLeaf *
rope<charT, Alloc>::leaf_concat_char_iter(RopeLeaf *r, const charT *iter, size_t len) {
...
template<class charT, class Alloc>
rope<charT, Alloc>::RopeLeaf *
rope<charT, Alloc>::destr_leaf_concat_char_iter(RopeLeaf *r, const charT *iter, size_t len) {
...
#ifndef __GC
			if (result != balanced) {
				__stl_assert(1 == result->refcount && 1 == balanced->refcount);
			}
#endif
			result->unref_nonnil();
		}
...
template<class charT, class Alloc>
rope<charT, Alloc>::RopeBase *rope<charT, Alloc>::concat_char_iter(RopeBase *r, const charT *s, size_t slen) {
	RopeBase *result;
	if (0 == slen) {
		ref(r);
		return r;
	...
	if (RopeBase::concat == r->tag && RopeBase::leaf == ((RopeConcatenation *) r)->right->tag) {
...
#ifndef __GC
template<class charT, class Alloc>
rope<charT, Alloc>::RopeBase *rope<charT, Alloc>::destr_concat_char_iter(RopeBase *r, const charT *s, size_t slen) {
	RopeBase *result;
	if (0 == r) return RopeLeaf_from_unowned_char_ptr(s, slen);
	...
	if (RopeBase::concat == r->tag) {
		RopeLeaf *right = (RopeLeaf *) (((RopeConcatenation *) r)->right);
		if (RopeBase::leaf == right->tag && right->size + slen <= copy_max) {
...
rope<charT, Alloc>::concat(RopeBase *left, RopeBase *right) {
	...
	if (RopeBase::leaf == right->tag) {
		...
		} else if (RopeBase::concat == left->tag && RopeBase::leaf ==
...
template<class charT>
bool __rope_insert_char_consumer<charT>::operator()(const charT *leaf, size_t n) {
	size_t i;
...
inline bool __rope_insert_char_consumer<char>::operator()(const char *leaf, size_t n) {
	size_t i;
...
inline bool __rope_insert_char_consumer<wchar_t>::operator()(const wchar_t *leaf, size_t n) {
	size_t i;
	for (i = 0; i < n; i++) o.put(leaf[i]);
	return true;
}
...
template<class charT>
inline bool __rope_is_simple(charT *) { return false; }
...
template<class charT, class Alloc>
void rope<charT, Alloc>::dump(RopeBase *r, int indent) {
...
rope<charT, Alloc>::balance(RopeBase *r) {
	...
	__STL_TRY {
		...
		for (i = 0; i <= RopeBase::max_rope_depth; ++i)
			if (0 != forest[i]) {
#ifndef __GC
...
template<class charT, class Alloc>
void rope<charT, Alloc>::add_to_forest(RopeBase *r, RopeBase **forest) {
	if (r->is_balanced) {
...
template<class charT, class Alloc>
void rope<charT, Alloc>::add_leaf_to_forest(RopeBase *r, RopeBase **forest) {
	...
	for (;; ++i) {
		...
		if (i == RopeBase::max_rope_depth || insertee->size < min_len[i + 1]) {
			forest[i] = insertee;
...
template<class charT, class Alloc>
charT rope<charT, Alloc>::fetch(RopeBase *r, size_type i) {
...
// flat strings.
template<class charT, class Alloc>
int rope<charT, Alloc>::compare(const RopeBase *left, const RopeBase *right) {
	size_t left_len;
	size_t right_len;
...
template<class charT, class Alloc>
charT rope<charT, Alloc>::empty_c_str[1];
...
template<class charT, class Alloc>
void __rope_rotate(__rope_iterator<charT, Alloc> first,
                   __rope_iterator<charT, Alloc> middle,
                   __rope_iterator<charT, Alloc> last) {
	__stl_assert(first.container() == middle.container() && middle.container() == last.container());
